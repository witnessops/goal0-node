...
def dry_run_evidence(plan: dict[str, Any]) -> dict[str, Any]:
    ...
    stdout = {
        "dry_run": True,
        "argv": plan.get("argv", []),
        "note": "Codex CLI was not executed.",
    }
    evidence = {
        "schema": "vaultsovereign.codex.execution_evidence.v1",
...
def execute_plan(
    ...
    if not argv or argv[0] != "codex" or len(argv) < 2 or argv[1] != "exec":
        raise ValueError("Run plan must render a codex exec argv vector")
    if not shutil.which("codex"):
        raise FileNotFoundError("codex binary not found on PATH; rerun with --dry-run or install Codex CLI")
