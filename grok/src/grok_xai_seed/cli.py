...
app = typer.Typer(no_args_is_help=True, help="Governed Grok CLI overlay.")
console = Console()
WOPS_ROOT = Path("/home/ops/witnessops-node")
DEFAULT_POLICY = WOPS_ROOT / "policies/grok_exec_policy.v1.json"
...
def validate_cmd(
    ...
    """Validate a governed Grok task bundle against local policy."""
    task_obj = read_json(task)
    policy_obj = _load_policy(policy)
    verdict = validate_task(task_obj, policy_obj)
    out = out or WOPS_ROOT / "evidence/grok_policy" / f"{task_obj.get('task_id', 'task')}.verdict.json"
    _write(out, verdict)
...
def render_cmd(
    ...
    """Render a grok headless argv plan. No Grok execution occurs."""
    task_obj = read_json(task)
    verdict_obj = read_json(verdict)
    policy_obj = _load_policy(policy)
    plan = render_plan(task_obj, verdict_obj, task_path=task, verdict_path=verdict, policy=policy_obj)
    _write(out, plan)
    console.print_json(data={"task_id": plan["task_id"], "plan_hash": plan["plan_hash"], "argv_preview": plan["argv"][:10] + ["..."]})
...
def run_cmd(
    ...
    """Dry-run or execute a rendered Grok run plan and capture evidence."""
